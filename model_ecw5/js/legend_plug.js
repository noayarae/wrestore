...
// ==========------------ Start: (2) Settings for PFR-Legend in Heatmap (original) ------------- ==================== //
...
function buttonControl_pfr_original(colorList, rg_pfr) {
...
//        alert("Line:2694-g2.php: " + "colors: " + Object.keys(colors).length + " map: " + map + "  min: " + min_v + "  max: " + max_v);
    ...
    // New DIV. This 'Div' is not needed for Modified
...
//            label.innerHTML = range_inf2 + ' to ' + range_sup2;// + ' cfs';
    ...
    heatmapPF1.controls[google.maps.ControlPosition.TOP_LEFT].push(hml_extraFrame);
...
// --------------------- End: (2) Settings for PFR-Legend in Heatmap (original) ---------------------------------- //


// ===============----------------- Start: (2) Settings for PFR-Legend in Heatmap (modified) ------------- ==================== //

//  /////////////  FUNCTION for PFR-LEGEND button into the heat-map  ///////////
// function buttonControl_pfr(options, colorList, map, min_v, max_v,rg_pfr) {
// function buttonControl_pfr(colorList, map, min_v, max_v, rg_pfr) {
function buttonControl_pfr(colorList, rg_pfr) {
    // Level 0: extra frame to engañar to google map
    var hml_extraFrame = document.createElement('DIV'); //E: main container of the main-map-legend pannel
    hml_extraFrame.className = "hml_extraFrame"; //E: class name for the created DIV
    ...
    var heatMapLegend_frame = document.createElement('DIV'); //E: main container of the main-map-legend pannel
    heatMapLegend_frame.className = "heatMapLegend_frame"; //E: class name for the created DIV
    heatMapLegend_frame.style.width = "50px";//E: Added for Modified Legend ---------------------
    heatMapLegend_frame.index = 1;//E: Added for Modified Legend ---------------------
    ...
    heatMapLegend_frameHead.title = 'Click here to On/Off the legend';
    heatMapLegend_frameHead.index = 1;
    heatMapLegend_frameHead.style.width = "300%";//E: Added for MODIFIED legend -----------
    heatMapLegend_frameHead.style.fontSize = "12px";//E: Added for MODIFIED legend --------


    // Level 1.b. Legend displayable contain (contenido desplegable)
    var heatMapLegend_container_pfr = document.createElement('DIV'); //E: container of labels and square boxes
    heatMapLegend_container_pfr.className = "heatMapLegend_container_pfr"; //E: class name for the created DIV
    heatMapLegend_container_pfr.style.display = "inline-grid";  //"block"; //E: Modified for MODIFIED legend ---------------
    heatMapLegend_container_pfr.title = 'Peak Flow Reduction in cubic meters per second';//E:adding TITLE
    heatMapLegend_container_pfr.style.paddingLeft = '3px';//E: Added for MODIFIED legend ---------------
    heatMapLegend_container_pfr.style.paddingTop = '10px';//E: Added for MODIFIED legend ---------------

    // Level 1.a.1
    var heatMapLeg_title = document.createElement('DIV'); //E: legend-header (title) DIV is created here
    // heatMapLeg_title.innerHTML = "Legend.";
    heatMapLeg_title.innerHTML = "Peak Flow Reduction in cubic meters per second.";
    heatMapLeg_title.className = 'heatMapLegend_title';
    heatMapLeg_title.style.width = '100%';//E: ADDED for Modified -------------------------------
    heatMapLeg_title.style.padding = '0px';//E: ADDED for Modified ------------------------------
    ...
    for (var key in colorList) {
        var boxContainer_M = document.createElement("DIV");
        boxContainer_M.style.margin = 0;//E: ADDED for Modified ------------------------------
        boxContainer_M.style.lineHeight = "13px";//E: ADDED for Modified ------------------------------
        //            boxContainer.id = "box_container";
        boxContainer_M.className = "box_container";

        var box = document.createElement("DIV");
        var box2 = document.createElement("DIV");//E: ADDED for Modified ------------------------------
        var label = document.createElement("SPAN");
...
//            var range_sup2 = min_v + Math.ceil((ii+1)*range2); // Get the high value of range
        ...
        boxContainer_M.appendChild(box);
        boxContainer_M.appendChild(box2);//E: ADDED for Modified ------------------------------
        boxContainer_M.appendChild(label);
        heatMapLegend_container_pfr.appendChild(boxContainer_M);

        // alert (ii);
        //E: ADDED for Modified ------------------------------
        var remain = (5*ii) % 10;
        if (remain == 0) {
            label.innerHTML = range_inf2;
            // alert (ii);
        } else {
            label.innerHTML = " ";
        }
        // label.innerHTML = range_inf2 + ' - ' + range_sup2;// + ' cfs';
        label.className = "label";
        label.style.position = "relative";//E: ADDED for Modified ------------------------------
        label.style.top = "-7px";//E: ADDED for Modified ------------------------------

        box2.className = "box2";//E: ADDED for Modified ------------------------------
        box2.style.backgroundColor = "#ffffff";//E: ADDED for Modified ------------------------------

        box.className = "box";
        box.style.backgroundColor = colorList[key];
        box.style.border = "none";//E: ADDED for Modified ------------------------------
        box.style.marginRight = "0px";//E: ADDED for Modified ------------------------------

        ii += 1;
    }
    heatMapLegend_container_pfr.appendChild(boxContainer_M);

    //Start: Add one more block at the bottom
    var last_box = document.createElement("DIV");//E: Added for Modified Legend ------
    last_box.className = "box";
    // last_box.style.backgroundColor = "#ffff44";//E: Added for Modified Legend ------
    last_box.style.border = "none";//       E: Added for Modified Legend ------
    last_box.style.marginRight = "0px";//   E: Added for Modified Legend ------

    var last_box2 = document.createElement("DIV");//E: Added for Modified Legend ------
    last_box2.className = "box2";
    last_box2.style.backgroundColor = "#ffffff";
    var last_label = document.createElement("SPAN");//E: Added for Modified Legend ------
    last_label.innerHTML = range_sup2;//rg_pfr[ii+1]
    last_label.style.position = "relative";
    last_label.style.top = "-7px";

    //E: This 'DIV' is for the last box (empty box)
    var boxContainer_M2 = document.createElement("DIV");
    boxContainer_M2.className = "box_container";
    boxContainer_M2.style.margin = 0;//E: ADDED for Modified ------------------------------
    boxContainer_M2.style.lineHeight = "13px";//E: ADDED for Modified ------------------------------

    boxContainer_M2.appendChild(last_box);
    boxContainer_M2.appendChild(last_box2);
    boxContainer_M2.appendChild(last_label);
    heatMapLegend_container_pfr.appendChild(boxContainer_M2);
    //End: Add one more block at the bottom
...
// --------------------- End: (2) Settings for PFR-Legend in Heatmap (modified) ---------------------------------- //



// ===========------------- Start: (3) Settings for PROFIT-Legend in Heatmap (ORIGINAL) ------------- ==================== //
...
function buttonControl_cr_original(colorList, rg_pr) {
    // min_v = Math.trunc(min_v);
//        alert("Line:2694-g2.php: " + "colors: " + Object.keys(colors).length + " map: " + map + "  min: " + min_v + "  max: " + max_v);
...
// ==============--------------- End: (3) Settings for Legend in PROFIT-Heatmap (ORIGINAL) ------------- =================== //


// ===========-------------- Start: (3) Settings for PROFIT-Legend in Heatmap (MODIFIED) ------------- ==================== //

//  /////////////  FUNCTION for COST-LEGEND button into the heat-map  ///////////
// function buttonControl_cr(options, colorList, map, min_v, max_v,rg_pr) {
// function buttonControl_cr(colorList, map, min_v, max_v,rg_pr) {
function buttonControl_cr(colorList, rg_pr) { //E: For MODIFIED
    // min_v = Math.trunc(min_v);
//        alert("Line:2694-g2.php: " + "colors: " + Object.keys(colors).length + " map: " + map + "  min: " + min_v + "  max: " + max_v);
    ...
    var heatMapLegend_frame = document.createElement('DIV'); //E: main container of the main-map-legend pannel
    heatMapLegend_frame.className = "heatMapLegend_frame"; //E: class name for the created DIV
    heatMapLegend_frame.style.width = "70px";//E: Added for Modified Legend ---------------------
    heatMapLegend_frame.index = 1;//E: Added for Modified Legend ---------------------
    ...
    var heatMapLegend_container_cr = document.createElement('DIV'); //E: container of labels and square boxes
    heatMapLegend_container_cr.className = "heatMapLegend_container_cr"; //E: class name for the created DIV
    heatMapLegend_container_cr.style.display = "inline-grid";//"block";//E: Modified for MODIFIED legend ---------------
    heatMapLegend_container_cr.title = 'Profit in US Dollars';//E:adding TITLE
    heatMapLegend_container_cr.style.paddingLeft = '3px';//E: Added for MODIFIED legend ---------------
    heatMapLegend_container_cr.style.paddingTop = '10px';//E: Added for MODIFIED legend ---------------

    // Level 1.a.1
    var heatMapLeg_title = document.createElement('DIV'); //E: legend-header (title) DIV is created here
    // heatMapLeg_title.innerHTML = "Legend+";
    heatMapLeg_title.innerHTML = "Profit in US Dollars";
    heatMapLeg_title.className = 'heatMapLegend_title';
    heatMapLeg_title.style.width = '100%';//E: ADDED for Modified -------------------------------
    heatMapLeg_title.style.padding = '0px';//E: ADDED for Modified ------------------------------

    // // New DIV //E: Not needed for Modified
    // var box_for_units = document.createElement('DIV'); //E: This DIV will host the units description
    // box_for_units.innerHTML = "Profit in US Dollars";
    // box_for_units.className = 'box_for_units';
    // box_for_units.style.fontSize = "10.5px";
...
//            var range_sup2 = min_v + Math.ceil((i+1)*range2); // Get the high value of range
        ...
        boxContainer_M.appendChild(box2);//E: ADDED for Modified ------------------------------
        boxContainer_M.appendChild(label);
        heatMapLegend_container_cr.appendChild(boxContainer_M);

        // // E: ADDED for Modified ------------------------------
        var remain = (5*ii) % 10;
        if (remain==0 || range_inf2==0) {
            label.innerHTML = range_inf2;
            // alert (ii);
        } else {
            label.innerHTML = " ";
    ...
    boxContainer_M2.appendChild(last_box2);
    boxContainer_M2.appendChild(last_label);
    heatMapLegend_container_cr.appendChild(boxContainer_M2);
    // //End: Add one more block at the bottom

    // //Level 1.a.2//E: Not needed  for Modified -------
    // var drop_arrow = document.createElement('DIV'); //E: DIV is created the dropdown arrow
    // drop_arrow.className = "dropdown_img"; //E: class name for the created DIV
    // drop_arrow.innerHTML = '<i style="font-size:20px" class="fa">&#xf102;</i>';
    ...
    heatMapLegend_frameHead.appendChild(heatMapLeg_title);
    // heatMapLegend_frameHead.appendChild(drop_arrow);//E: Eliminate for MODIFIED ---------


    //E: Add a control for "hml_extraFrame" DIV. Definition and Settings of "hml_extraFrame" DIV is above
    // heatmapRV1.controls[google.maps.ControlPosition.TOP_RIGHT].push(hml_extraFrame);
    heatmapRV1.controls[google.maps.ControlPosition.TOP_LEFT].push(hml_extraFrame);
...
// ==============--------------- End: (3) Settings for Legend in PROFIT-Heatmap (MODIFIED) ------------- =================== //
...
function buttonControl_sr(colorList, rg_sr) {
    // min_v = Math.trunc(min_v);
//        alert("Line:2694-g2.php: " + "colors: " + Object.keys(colors).length + " map: " + map + "  min: " + min_v + "  max: " + max_v);
...
function buttonControl_nr(colorList, rg_nr) {
    // min_v = Math.trunc(min_v);
//        alert("Line:2694-g2.php: " + "colors: " + Object.keys(colors).length + " map: " + map + "  min: " + min_v + "  max: " + max_v);
