...
function subBasin_barplot_left() {
    ...
    var option_sb = document.getElementById('subDrop').value;
    ...
    if (option_sb == "Watershed") {
        ...
        chart = new google.visualization.BarChart(document.getElementById('chart_bp1'));
...
// ---------------------- else
        ...
        for (i = 0; i < numberOf_alternatives; i++) {
            ...
            $.each(array_fullvalues[rowlen], function(key, value) {
                // if (key == option) {
                if (key == option_sb) {
                    // alert("L.479 : "+key + ': ' + value);
                    //subChart=value.split(',');
                    subChart = JSON.parse("[" + value + "]");
                }
            });
            //JSON.parse(chartArray[rowlen].F1)[0]
            // E: subChart[1] corresponds to PFR
            var m11_left = Number(parseFloat(Math.abs(subChart[1][0]))); //E: MEAN value
            var min11_left = Number(parseFloat(Math.abs(subChart[1][1]))); //E: MiN value
            var max11_left = Number(parseFloat(Math.abs(subChart[1][2]))); //E: MAX value
            var perm11_left = 0.0006;
            var perm11_left = Number(Math.abs(parseFloat(m11 / (JSON.parse(fn_obj_array[rowlen].F1)[0])))) * 100;
            ...
            //E: For Deterministic
            // alert ("L.527 test: "+ m11_left);
            data.addRow([v_label,m11_left,
                'bar {fill-color:'+colors[rowlen]+';fill-opacity:'+1+'; stroke-color:'+border[rowlen]+';stroke-width:'+2+'},'+'line {stroke-color:'+'#000066'+'; stroke-width:'+1+'}',
                HTMLContent_for_deterministic(rowlen+1,'PFR',m11_left.toFixed(2),'PFR wrt Watershed',perm11_left.toExponential(2))
            ]);
            //tool tip edited for only PFR (see above)
            function HTMLContent_for_deterministic(row, dm, m, min, max) {
                // return '\n' + 'Alternative+: ' + row + '\n' + dm + ': ' + m + "\n min:" + min + "\nmax:" + max; //E: For Non deterministic
                // return '\n' + 'Alternative+: ' + row + '\n' + dm + ': ' + m;    //E: For Deterministic
                return '\n' + dm + ': ' + m +" cms";    //E: For first experiment
        ...
        var options = {////E: For 'PFR',
            ...
            //E: This controls the gap distance of the plot from the top and left border
            chartArea: {top:0, left:60},//chartArea: {top:25, left:50},
        ...
        // chart = new google.visualization.BarChart(document.getElementById('chart_div1')chart_bp1);
        chart1_bpl = new google.visualization.BarChart(document.getElementById('chart_bp1'));
        chart2_bpl = new google.visualization.BarChart(document.getElementById('chart_div2'));
        chart3_bpl = new google.visualization.BarChart(document.getElementById('chart_div3'));
        chart4_bpl = new google.visualization.BarChart(document.getElementById('chart_div4'));

        chart1_bpl.draw(data, options); //chart.draw(data, options);
        chart2_bpl.draw(data1, options1); //chart1.draw(data1, options1);
        chart3_bpl.draw(data2, options2); //   chart2.draw(data2, options2);
        chart4_bpl.draw(data3, options3); //chart3.draw(data3, options3);

        //the barclick event capturing is done here.
        // -------------------------------------- chart - SUB BASIN ---------------------------//
        google.visualization.events.addListener(chart1_bpl, 'select', function () {
            // "chart1_bpl.getSelection()" gets the selected Suggested-Plan
            var clickd = chart1_bpl.getSelection();
            var selected_item = chart1_bpl.getSelection()[0];

            // alert("L.780 - chart-1 SB: " + clickd.length);
            // alert("L.783 - chart-1 W2: "+ JSON.stringify(selected_item));
            if (clickd.length === 0) {
                report('m-clk%  ' , 'Un-selection of alternative SB ' + ';');
            }
            else {
                var topping = data.getValue(selected_item.row,0);
                report('m-clk%  ' , 'Alternative SB '+ topping , ';');
                // alert("L.787- topping: "+topping);
            }
        });

        // -------------------------------------- chart 1 - SUB BASIN ---------------------------//
        google.visualization.events.addListener(chart2_bpl, 'select', function () {
        // google.visualization.events.addListener(chart1, 'select', function () {
            var clickd = chart1.getSelection();
            var selected_item = chart1.getSelection()[0];
        ...
        // -------------------------------------- chart 2 - SUB BASIN ---------------------------//
        google.visualization.events.addListener(chart3_bpl, 'select', function () {
        // google.visualization.events.addListener(chart2, 'select', function () {
            var clickd = chart2.getSelection();
            var selected_item = chart2.getSelection()[0];
        ...
        // -------------------------------------- chart 3 - SUB BASIN ---------------------------//
        google.visualization.events.addListener(chart4_bpl, 'select', function () {
        // google.visualization.events.addListener(chart3, 'select', function () {
            var clickd = chart3.getSelection();
            var selected_item = chart3.getSelection()[0];
